...
<div >
        ...
        if ($result-> num_rows > 0){
            while ($row=$result-> fetch_assoc()) {
                ...
                <tr>
                    ...
                    <td><?=$row["phone"]?></td>
                    <td><?=$row["city"]?></td>
                    <td><img height='100px' src='uploaded_img_dalel/<?=$row["photo"]?>'></td>
                    <td><?=$row["bd"]?></td>


                    <td>

                  <a href="deletdalel.php?id=<?php echo $row["idd"] ?>" class="link-dark" style="color:#8c51a5 "><i class="fa-solid fa-trash fs-5"></i></a>
                        <a href="editdalel.php?id=<?php echo $row["idd"] ?>" class="link-dark"style="color:#8c51a5 "><i class="fa-solid fa-pen-to-square fs-5 me-3"></i></a>
    ...
    <script>
        function myFunction() {
            window.location.href="adddalel.php";
        }
    </script>
    <div>
        <button type="button" class="btn btn-secondary" style="height:50px;width:150px;background-color:#9078c0 ;border-radius: 20px;" data-toggle="modal" data-target="#myModal" onClick="myFunction()" >
            <div style="color: white"> Add a tourist guide </div>
        </button>
    </div>
</div>
